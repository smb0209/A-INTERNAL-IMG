...
3. 주소 입력: `192.168.x.x:8080`
   - MSX 앱은 슬래시(`/`) 입력이 안 되므로 **`/menu.json` 없이** 호스트:포트만 넣으면 됩니다.
     (서버가 루트 `/` 요청에도 동일한 슬라이드쇼 JSON을 내려줍니다. `http://` 도 MSX가 자동으로 붙임)
...
- MSX는 호스트만 입력하면 **`/msx/start.json`** 을 요청 → 서버가 **Start Object**(`parameter: content:.../menu.json`)로 응답 → `menu.json`(슬라이드쇼 content)을 다시 로드. content/이미지 URL은 **요청 Host 헤더**로 생성해 TV가 접속한 주소와 항상 일치.
- 모든 요청은 콘솔 + `C:\adImg\access.log` 에 기록(디버깅용).
- Windows 7 / PowerShell 2.0 호환 (`ConvertTo-Json` 미사용, 수동 JSON 생성).
- `HttpListener` 의 LAN 바인딩 때문에 **관리자 권한 필요** → exe가 자동 승격(UAC).
- 시작 시 **방화벽 인바운드(TCP 8080)를 자동 허용** (`netsh advfirewall`). TV/다른 PC 접속용.
  자동설정 실패 시 수동: `netsh advfirewall firewall add rule name="A-INTERNAL-IMG 8080" dir=in action=allow protocol=TCP localport=8080`
- LAN IP 자동 감지, CORS 허용, MIME Content-Type 지정.

## 설정 변경 / 한 PC에서 여러 대 운영 (config.txt)

exe를 다시 빌드하지 않고 **`config.txt`** 로 설정을 덮어쓸 수 있습니다.
`A-INTERNAL-IMG.exe` 와 **같은 폴더**에 `config.txt` 를 두면 자동 적용됩니다.
(릴리스의 `config.example.txt` 를 받아 `config.txt` 로 이름 바꿔 사용)
...
**한 PC에서 2대 이상** (서로 다른 매장/드라이브/포트):

```text
C:\signage1\  ->  A-INTERNAL-IMG.exe + config.txt(Port=8080, BaseDir=C:\signage1\data, DriveFolderId=AAA)
C:\signage2\  ->  A-INTERNAL-IMG.exe + config.txt(Port=8081, BaseDir=C:\signage2\data, DriveFolderId=BBB)
```

→ 두 exe를 각각 실행, TV에는 `PC아이피:8080` / `PC아이피:8081` 입력. 방화벽·데이터폴더는 자동으로 분리됩니다.

> 즉시 조절용 파일(BaseDir 안에 두면 서버 재시작 없이 다음 루프 반영):
> `duration.txt`(노출초), `fade.txt`(페이드 ms), `transition.txt`(`none` 이면 크로스페이드 끄고 기존 방식)
...
| `$MaxImageWidth` | `1920` | 동기화 시 이미지 가로 최대폭(px). 4K 등 대형 이미지 자동 축소 (0=축소안함) |
